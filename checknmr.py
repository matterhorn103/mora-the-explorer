...
# Checks that two spectra with the same name are actually identical and not e.g. different proton measurements
def identical_spectra(mora_folder, dest_folder):
    ...
    # The first spectrum with a given title is always copied, so it is possible that it doesn't have an audit file
    except FileNotFoundError:
        exp_dest = None
...
def check_nmr(fed_options, check_day, mora_path, spec_paths, wild_group, prog_bar, progress_callback):
    ...
    if spectrometer == "300er":
        ...
        # Code to check folders of all groups when the nmr group is chosen and the wild group option is invoked
        if wild_group is True:
            check_list = []
            # Slightly complicated bit of code here but it just goes through all folders in 500-600er folder
            logging.info("The following paths will be checked:")
        ...
        # Normal behaviour for all other users
            ...
            # Try to get list of spectrum folders in folder for requested spectrometer, group and date
    ...
    if spectrometer == "300er" or spectrometer == "400er":
        ...
        for check_path in check_path_list:
            ...
            for folder in check_list:
                ...
                # Check if spectrum is a match for search term, including the wild option for the nmr group
                if (split_title[1][0:3] == fed_options["initials"]) or ((wild_group is True) and (split_title[2][0:3] == fed_options["initials"])):
                    logging.info("Spectrum matches search query!")
                    # Formatting options specifically for nmr group, include everything - even date and spec via parent folder name
                    if fed_options["group"] == "nmr":
                        new_folder_name = (
                            ("-".join(split_title[1:])) + "-" + ("-".join(split_details[:2])) + "_" + check_path.name + "_" + folder.name
                        )
                    # Otherwise format spectrum name according to user's choices
                    else:
                        # Format in the style of NMRCheck if requested i.e. using underscores, including initials and spectrometer and date and exp no
                        if fed_options["nmrcheck_style"] is True:
                            if len(split_title) > 2:
                                hyphenated_title = "_".join(split_title[1:]) + "_" + check_path.name + "_" + folder.name
                            else:
                                hyphenated_title = "_".join(split_title) + "_" + check_path.name + "_" + folder.name
                        # Length checks above and below are to account for the possibility that the user might have forgotten to separate with spaces
                        # Principle applied is that if the information being dropped isn't 100% definitely what we think it is (i.e. the group name), play it safe and don't drop it
                        ...
                        # Append experiment type e.g. proton to end of name, and solvent if requested
                    ...
                    if new_folder_path.exists() is True:
                        ...
                        # Check that the spectra are actually identical and not e.g. different proton measurements
                        # If confirmed to be unique spectra, need to extend spectrum name with -2, -3 etc. to avoid conflict with spectra already in dest
                        ...
                        if identical_spectra(folder, new_folder_path) is False:
                            ...
                            while new_folder_path.exists() is True:
                                # Do whole procedure again as long as name has a match in the destination
                                if identical_spectra(folder, new_folder_path) == True:
                    ...
                    # Otherwise there is no existing spectrum in the destination so straightforward copy
                    else:
                        try:
                            shutil.copytree(folder, new_folder_path)
                        except PermissionError:
        ...
        for folder in check_list:
            logging.info(folder)
            # Check for initials at start of folder name, as folders are given the name of the sample on 500 and 600 MHz spectrometers
            if folder.name[:3] == fed_options["initials"]:
                ...
                if fed_options["group"] == "nmr":
                    new_folder_name = folder.parent.parent.name + "_" + folder.parent.name + "_" + folder.name + "_" + magnet_freq
                elif fed_options["nmrcheck_style"] is True:
                    new_folder_name = fed_options["initials"] + "_" + folder.name[3:] + "_" + magnet_freq
                elif fed_options["inc_init"] is True:
                    new_folder_name = fed_options["initials"] + "-" + folder.name[3:] + "_" + magnet_freq
                else:
                    new_folder_name = folder.name[3:] + "_" + magnet_freq
                new_folder_path = Path(fed_options["dest_path"]) / new_folder_name
                # Check that spectrum hasn't been copied before
                # Begin by setting check number to >0 so that if nothing has ever been copied the spectrum gets copied
                new_spectra = True
                partial_copy = False
                if new_folder_path.exists() is True:
                ...
                if new_spectra is True:
                    if partial_copy is True:
                        ...
                        output_list.append(text_to_add)
                    logging.info(f"Spectrum saved to {new_folder_path}")
                # Make progress bar move noticeably while checking/copying users' spectra so it doesn't look like it has crashed
                max_progress += 20
                prog_bar.setMaximum(max_progress)
                prog_state += 20
                progress_callback.emit(prog_state)
