...
The package and app are configured using a file in TOML format.
...
Any of the options can be specified in the user config file, with the exception of the `admin` table, which is only read from the app config file.

Note that some options only affect the behaviour of the GUI desktop app.

---

`version` – The config file version (`integer`); if the user config version doesn't match the app config, it is deleted
