...
if platform.system() == "Darwin" and getattr(sys, "frozen", False):
    rsrc_dir = Path(sys._MEIPASS)
else:
    rsrc_dir = Path(__file__).parent
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Check for updates
        update_return = self.update_check()
        if update_return == "permission_error":
            sys.exit()
        ...
        # If a config file from version <1.4.9 exists in program folder, copy it across to the proper location. Delete later, after everyone has migrated
        elif old_config_path.exists() is True:
            self.config_file_path.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(old_config_path, self.config_file_path)
            with open(self.config_file_path, encoding="utf-8") as config_file:
                self.options = json.load(config_file)
        else:
            self.options = {
        ...
        # For users upgrading from <1.5.2, add nmrcheck_style to options
        if "nmrcheck_style" not in self.options:
            self.options["nmrcheck_style"] = False
            with open(self.config_file_path, "w", encoding="utf-8") as config_file:
                json.dump(self.options, config_file)

        # Load path to mora from file. If none is found, set to default path on Windows, or require user input on other OSes
        self.mora_location_file_path = rsrc_dir / "mora_path.txt"
        if self.mora_location_file_path.exists() is True:
            with open(self.mora_location_file_path, encoding="utf-8") as mora_location_file:
                mora_path_dict = {}
                mora_path_dict["Windows"] = Path(mora_location_file.readline().rstrip())
                mora_path_dict["Darwin"] = Path(mora_location_file.readline().rstrip())
                mora_path_dict["Linux"] = Path(mora_location_file.readline().rstrip())
                self.mora_path = mora_path_dict[platform.system()]

        # Load available groups from groups.json
        with open(rsrc_dir / "groups.json", encoding="utf-8") as groups_file:
            self.groups_dict = json.load(groups_file)
        ...
        # Add radio button for each group in groups_dict (loaded earlier)
        self.AKlist = list(self.groups_dict.keys())
        ...
        self.AKlist_other = list(self.groups_dict["other"])
    ...
    # Define function to check for updates at location specified in update_path.txt
    def update_check(self):
        logging.info("Checking for updates at:")
        with open(rsrc_dir / "update_path.txt", encoding="utf-8") as update_path_file:
            self.update_path = Path(update_path_file.readline().rstrip())
            update_path_version_file = self.update_path / "version.txt"
            logging.info(self.update_path)
        with open(rsrc_dir / "version.txt", encoding="utf-8") as version_file:
            self.version_no = version_file.readlines()[2].rstrip()
            logging.info(f"Current version: {self.version_no}")
        try:
...
The repeat function is also disabled as long as this option is selected.
        ...
        if self.since_button.isChecked() is True and self.options["group"] != "nmr":
            since_warning = QMessageBox.warning(self, "Warning", since_message)
    ...
    def group_changed(self):
        if self.AK_button_group.checkedButton().text() == "other":
            ...
            self.path_hf = (
                self.mora_path / "500-600er" / self.groups_dict["other"][self.options["group"]]
            )
        else:
            self.other_box.hide()
...
# Create instance of MainWindow as defined above, then show it
window = MainWindow()
window.show()


# Make dark mode less black because Windows dark mode looks bad
